...
int	pipcount;		/* Total number of live pipes 		*/
...
static	void	sysinit(void)
{
	...
	// originally putting pipes before sems.... all sem.count = 0.... :((((
	...
	for (i = 0; i < NPIPE; i++) {
		...
		piptr->emptySem = semcreate(0);
		piptr->fullSem = semcreate(PIPE_SIZE);
		
		//kprintf("__(pipid=%d)___(emptySem=%d, fullSem=%d)___\r\n", i, piptr->emptySem, piptr->fullSem);
