...
#define TEST3_OFFSET 0
static void _receiver3(void){
  int n;
  for(n = 0; n< 10; n++ ){
    receiveq();
    //kprintf("main received %d\r\n", msg);
  }
}
static void _sender3(int pid){
  int n;
  for(n = 0; n< 10; n++ ){
    sendq( pid, n+TEST3_OFFSET );
    //kprintf("sent %d\r\n", n+TEST3_OFFSET);
  }
}

void test3(void){
  kprintf("Test 3 -- One sender one receiver. multiple messages per sender. 10 points\r\n");
  pid32 pid = create(_receiver3, 1024, 10, "receiver3", 0);
  //kprintf("main pid for receiver is = %d\r\n", pid);
  //resume(pid);
  resume(create(_sender3, 1024, 15, "sender3", 1, pid));
  //int n = 0;
  //for (n = 0; n < 10; n ++) {
  	resume(pid);
  //}
...
void extra_test(void) {

  sema = semcreate(0);

  kprintf("======================Start extra Testing=====================\r\n");

  test3();

  kprintf("======================End of extra Test=====================\r\n");
  return ;
}
