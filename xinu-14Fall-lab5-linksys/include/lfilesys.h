...
#define LF_PATH_DEPTH	20		/* relative depth of the file to root */
...
#define LF_AREA_ROOT 0    	/* first sector that store info about root */

#define LF_TYPE_DIR	1 		/* dir type 			*/
#define LF_TYPE_FILE	0		/* file type			*/
...
struct	ldentry	{			/* description of entry for one	*/
	...
	int 	ld_type;		/* is a file or dir 		*/
	//struct 	ldentry	ld_parent_dir;	/* parent directory 	*/
	bool8	isOccupied;			/* is the entry currently allocated or free */
...
struct	lfdir	{			/* entire directory on disk	*/
	...
	// still need when traverse
	int32	lfd_nfiles;		/* current number of files	*/
	struct	ldentry lfd_files[LF_NUM_DIR_ENT]; /* set of files	*/
	//struct 	ldentry lfd_root; // make the first entry in dir "root"
	char	padding[20];		/* unused chars in directory blk*/
	ibid32 	lfd_ifirst; 		/* lists of file name, index number mapping */
